...
function _init()
  ...
  -- power shield system (Level 3 only)
  enemy_power_shield_active = false
  enemy_shield_timer = 0
  enemy_shield_duration = 300  -- 5 seconds
  
  -- Level 1 invisibility system
  enemy_invisible = false
  enemy_visibility_timer = 360  -- 6 seconds visible initially
  enemy_visible_duration = 360  -- 6 seconds visible
  enemy_invisible_duration = 180  -- 3 seconds invisible
  ...
  enemy_shot_counter = 0  -- track shots fired
...
function _update()
  ...
  if game_state == "level_select" then
    ...
    if level_select_confirmed then
      ...
      if level_confirm_timer <= 0 then
        ...
        -- initialize power shield for Level 3
        if current_level == 3 then
          enemy_power_shield_active = true
          enemy_shield_timer = 0
        else
          enemy_power_shield_active = false
          enemy_shield_timer = 0
        ...
        -- reset Level 2 special weapon counter
  ...
  -- update power shield timer (Level 3)
  if current_level == 3 and enemy_shield_timer > 0 then
    enemy_shield_timer = enemy_shield_timer - 1
    if enemy_shield_timer == 0 then
      enemy_power_shield_active = true  -- restore after 5 seconds
    end
  end
  ...
  if current_level == 1 and not enemy_defeated then
    ...
    if enemy_visibility_timer <= 0 then
      if enemy_invisible then
        ...
        enemy_invisible = false
        enemy_visibility_timer = enemy_visible_duration  -- 6 sec
      else
        -- switch to invisible
        enemy_invisible = true
        sfx(23)
        enemy_visibility_timer = enemy_invisible_duration  -- 3 sec
      end
    end
  end
...
-- handle power shield for Level 3
        if current_level == 3 and enemy_power_shield_active then
          enemy_power_shield_active = false  -- deactivate shield
          enemy_shield_timer = enemy_shield_duration  -- start 5-second timer
        else
        -- enemy takes damage (normal behavior or Level 3 no shield)
  ...
  for bullet in all(enemy_bullets) do
    ...
    -- check collision with rocks (normal bullets only - big shots pass through)
    if not bullet.is_big_shot then
      for rock in all(rocks) do
        if bullet.x < rock.x + 8 and bullet.x + 8 > rock.x and
           bullet.y < rock.y + 8 and bullet.y + 8 > rock.y then
...

__map__
